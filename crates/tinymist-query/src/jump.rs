    ...
    fn select_in_frame(
        ...
        for &(mut pos, ref item) in frame.items() {
            if self.priority == Some(0)
                && self.positions.iter().any(|position| position.page == page)
            {
                break;
            }
            ...
            match item {
                ...
                FrameItem::Text(text) => {
                    ...
                    for glyph in &text.glyphs {
                        ...
                        let priority = if self.priority == Some(0) {
                            (self.span == Some(span)).then_some(0)
                        } else {
                            interesting_spans.priority_of(span)
                        };
