...
mod post_type_check_tests {
    ...
    fn test() {
        snapshot_testing("post_type_check", &|ctx, path| {
            ...
            let node = typst_linked_node_leaf_at!(root, pos + 1).unwrap();
...
mod matcher_tests {
    ...
    fn test() {
        snapshot_testing("match_def", &|ctx, path| {
            ...
            let root = LinkedNode::new(source.root());
            let node = typst_linked_node_leaf_at!(root, pos).unwrap();
...
mod signature_tests {
    ...
    fn test() {
        snapshot_testing("signature", &|ctx, path| {
            ...
            let root = LinkedNode::new(source.root());
            let callee_node = typst_linked_node_leaf_at!(root, pos).unwrap();
...
mod call_info_tests {
    ...
    fn test() {
        snapshot_testing("call_info", &|ctx, path| {
            ...
            let root = LinkedNode::new(source.root());
            let mut call_node = typst_linked_node_leaf_at!(root, pos + 1).unwrap();
