    ...
    fn node(&mut self, id: NodeId, node: &CfNode, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match &node.instr {
            ...
            CfInstr::Contextual(index_vec_idx) => {
                write!(f, " = context ^{index_vec_idx}")?;
            }
            CfInstr::Show(cf_show) => {
                write!(f, " = show ^{}", cf_show.cont)?;
