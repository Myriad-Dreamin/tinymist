...
impl TinymistLanguageServer {
    ...
    pub fn get_completions(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        // TODO: Implement basic syntax-based completion when semantic context is not
        // available
        let js_completions = Array::new();

        // Basic Typst syntax keywords that can be completed
        let keywords = [
            "let", "set", "show", "import", "include", "if", "else", "for", "in", "while", "break",
            "continue", "return", "auto", "none", "true", "false",
        ];
    ...
    pub fn get_hover(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        use tinymist_query::{to_typst_position, HoverRequest, LspPosition, PositionEncoding};
    ...
    pub fn goto_definition(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        use tinymist_query::{
            to_typst_position, GotoDefinitionRequest, LspPosition, PositionEncoding,
            StatefulRequest,
        };
    ...
    pub fn find_references(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        use tinymist_query::{
            to_typst_position, LspPosition, PositionEncoding, ReferencesRequest, StatefulRequest,
        };
    ...
    pub fn folding_range(&self, uri: String) -> JsValue {
        ...
        if let Some(folding_ranges) = request.request(&source, PositionEncoding::Utf16) {
            ...
            for range in folding_ranges {
                ...
                js_sys::Reflect::set(&js_range, &"startLine".into(), &range.start_line.into())
                    .unwrap();
                js_sys::Reflect::set(&js_range, &"endLine".into(), &range.end_line.into()).unwrap();

                if let Some(start_char) = range.start_character {
                    js_sys::Reflect::set(&js_range, &"startCharacter".into(), &start_char.into())
                        .unwrap();
                }
                if let Some(end_char) = range.end_character {
                    js_sys::Reflect::set(&js_range, &"endCharacter".into(), &end_char.into())
                        .unwrap();
                }
    ...
    pub fn selection_range(&self, uri: String, positions: JsValue) -> JsValue {
        ...
        for i in 0..positions_array.length() {
            ...
            if let Some(line) = js_sys::Reflect::get(&pos_obj, &"line".into())
                .ok()
                .and_then(|v| v.as_f64())
                .map(|v| v as u32)
            {
                if let Some(character) = js_sys::Reflect::get(&pos_obj, &"character".into())
                    .ok()
                    .and_then(|v| v.as_f64())
                    .map(|v| v as u32)
                {
        ...
        use tinymist_query::{PositionEncoding, SelectionRangeRequest, SyntaxRequest};
    ...
    pub fn document_highlight(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        use tinymist_query::{to_typst_position, LspPosition, PositionEncoding};
        use typst_shim::syntax::LinkedNodeExt;
                ...
                if matches!(node.kind(), typst::syntax::SyntaxKind::Ident) {
                    ...
                    fn find_matching_idents(
                            ...
                            && node.text() == target
                        {
                            ...
                            let end_pos = source
                                .byte_to_line(node.offset() + node.text().len())
                                .unwrap();

                            js_sys::Reflect::set(
                                &start_obj,
                                &"line".into(),
                                &(start_pos as u32).into(),
                            )
                            .unwrap();
                            js_sys::Reflect::set(&start_obj, &"character".into(), &(0u32).into())
                                .unwrap(); // Simplified
                            js_sys::Reflect::set(
                                &end_obj,
                                &"line".into(),
            ...
            if node.children().count() == 0 {
                ...
                if text_len > 0 {
                    ...
                    tokens.extend_from_slice(&[
                        line_delta, char_delta, text_len, token_type, 0u32, // Token modifiers
                    ]);
                }
            } else {
    ...
    pub fn semantic_tokens_delta(&self, uri: String, previous_result_id: String) -> JsValue {
        ...
        use tinymist_query::{PositionEncoding, SemanticRequest, SemanticTokensDeltaRequest};
    ...
    pub fn document_color(&self, uri: String) -> JsValue {
        ...
        use tinymist_query::{DocumentColorRequest, PositionEncoding, SemanticRequest};
    ...
    pub fn document_link(&self, uri: String) -> JsValue {
        ...
        use tinymist_query::{DocumentLinkRequest, PositionEncoding, SemanticRequest};
    ...
    pub fn color_presentation(
        ...
        // Parse color from JsValue (assuming it's an LSP Color object with r,g,b,a
        // properties)
            ...
            for presentation in presentations {
                ...
                if let Some(text_edit) = presentation.text_edit {
                    ...
                    js_sys::Reflect::set(
                        &js_start,
                        &"line".into(),
                        &text_edit.range.start.line.into(),
                    )
                    .unwrap();
                    js_sys::Reflect::set(
                        &js_start,
                        &"character".into(),
                        &text_edit.range.start.character.into(),
                    )
                    .unwrap();
                    let js_end = Object::new();
                    js_sys::Reflect::set(&js_end, &"line".into(), &text_edit.range.end.line.into())
                        .unwrap();
                    js_sys::Reflect::set(
                        &js_end,
                        &"character".into(),
                        &text_edit.range.end.character.into(),
                    )
                    .unwrap();
                    ...
                    js_sys::Reflect::set(
                        &js_text_edit,
                        &"newText".into(),
                        &text_edit.new_text.into(),
                    )
                    .unwrap();

                    js_sys::Reflect::set(&js_presentation, &"textEdit".into(), &js_text_edit)
                        .unwrap();
                }
            ...
            let diagnostics = js_sys::Reflect::get(&ctx_obj, &"diagnostics".into())
                ...
                .and_then(|v| {
                    if v.is_undefined() || v.is_null() {
                        ...
                        Some(vec![]) // For WASM, we'll use empty diagnostics
                                     // for now
                    }
                })
                .unwrap_or_default();
    ...
    pub fn code_lens(&self, uri: String) -> JsValue {
        ...
        use tinymist_query::{CodeLensRequest, PositionEncoding, SemanticRequest};
    ...
    pub fn signature_help(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        use tinymist_query::{
            LspPosition, PositionEncoding, SemanticRequest, SignatureHelpRequest,
        };
    ...
    pub fn rename(&self, uri: String, line: u32, character: u32, new_name: String) -> JsValue {
        ...
        use tinymist_query::{LspPosition, RenameRequest, StatefulRequest};
    ...
    pub fn prepare_rename(&self, uri: String, line: u32, character: u32) -> JsValue {
        ...
        use tinymist_query::{LspPosition, PrepareRenameRequest, StatefulRequest};
    ...
    pub fn symbol(&self, pattern: String) -> JsValue {
        ...
        use tinymist_query::{SemanticRequest, SymbolRequest};
    ...
    pub fn on_enter(
        ...
        use lsp_types::{Position, Range};
        use tinymist_query::{OnEnterRequest, PositionEncoding, SyntaxRequest};
        ...
        let request = OnEnterRequest { path, range };
        ...
        if let Some(text_edits) = request.request(&source, PositionEncoding::Utf16) {
            ...
            for edit in text_edits {
                ...
                js_sys::Reflect::set(&start_obj, &"line".into(), &edit.range.start.line.into())
                    .unwrap();
                js_sys::Reflect::set(
                    &start_obj,
                    &"character".into(),
                    &edit.range.start.character.into(),
                )
                .unwrap();
                js_sys::Reflect::set(&end_obj, &"line".into(), &edit.range.end.line.into())
                    .unwrap();
                js_sys::Reflect::set(
                    &end_obj,
                    &"character".into(),
                    &edit.range.end.character.into(),
                )
                .unwrap();
                        ...
                        if let Ok(rename_obj) = file_rename.dyn_into::<js_sys::Object>() {
                            ...
                            if let (Ok(old_uri), Ok(new_uri)) = (
                                ...
                                if let (Some(old_str), Some(new_str)) =
                                    (old_uri.as_string(), new_uri.as_string())
                                {
    ...
    fn document_symbol_to_js(&self, symbol: &DocumentSymbol) -> Option<Object> {
        ...
        js_sys::Reflect::set(
            &start_obj,
            &"character".into(),
            &symbol.range.start.character.into(),
        )
        .ok()?;
        ...
        js_sys::Reflect::set(
            &end_obj,
            &"character".into(),
            &symbol.range.end.character.into(),
        )
        .ok()?;
    ...
    fn selection_range_to_js(&self, range: &lsp_types::SelectionRange) -> Object {
        ...
        js_sys::Reflect::set(
            &start_obj,
            &"character".into(),
            &range.range.start.character.into(),
        )
        .unwrap();
        ...
        js_sys::Reflect::set(
            &end_obj,
            &"character".into(),
            &range.range.end.character.into(),
        )
        .unwrap();
