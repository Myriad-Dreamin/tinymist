...
use sync_lsp::{just_future, LspClient, RequestId, SchedulableResponse};
...
pub struct TraceReport {
    request: TraceParams,
    messages: Vec<sync_lsp::LspMessage>,
    stderr: String,
}
...
async fn run_trace_program(params: TraceParams) -> anyhow::Result<JsonValue> {
    ...
    std::thread::spawn(move || {
        ...
        let messages = std::iter::from_fn(|| {
            ...
            let msg = sync_lsp::lsp::Message::read(&mut input_chan).ok()?;
            if let Some(sync_lsp::lsp::Message::Response(resp)) = &msg {
                if resp.id == 0.into() {
                    has_response = true;
                }
            }
...
async fn trace_main(
    ...
    client.send_notification_(sync_lsp::lsp::Notification {
        method: "tinymistExt/diagnostics".to_owned(),
        params: serde_json::json!(diagnostics),
    });
    match rpc_kind {
        "lsp" => {
            client.respond(sync_lsp::lsp::Response {
                id: req_id,
