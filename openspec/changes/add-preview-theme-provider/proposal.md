...
- Add a single `tinymist.preview.provider` setting for VS Code preview theme selection.
- Interpret `tinymist.preview.provider` values starting with `html:` as a filesystem path to an HTML entry file compatible with `tools/typst-preview-frontend`.
- Interpret any other non-empty `tinymist.preview.provider` value as an installed extension id that supplies preview HTML.
...
### New Capabilities
- `preview-theme-provider`: Let the VS Code extension resolve preview HTML from a built-in source, an external provider extension, or a configured local HTML file selected through a single `tinymist.preview.provider` setting with compatibility and trust checks.
...
- `editors/vscode/src/config.ts` and `editors/vscode/package.json` will need the new `tinymist.preview.provider` setting and provider-string parsing for `html:` paths.
