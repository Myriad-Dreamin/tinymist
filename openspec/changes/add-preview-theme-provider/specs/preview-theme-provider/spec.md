...
The VS Code Tinymist extension SHALL allow trusted workspaces to override the default preview HTML by setting `tinymist.previewer`. If the configured provider value cannot be resolved, Tinymist SHALL fall back to its built-in preview HTML.
...
The VS Code Tinymist extension SHALL interpret non-empty `tinymist.previewer` values that do not start with `html:` as extension ids for preview theme providers.
