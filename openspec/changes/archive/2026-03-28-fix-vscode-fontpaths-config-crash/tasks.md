...
- [x] 1.3 Add a deduplicated user-facing warning for malformed `tinymist.fontPaths` values that explains the expected `["fonts"]` JSON shape.

## 2. Validate the fix

- [x] 2.1 Add VS Code extension tests covering valid `fontPaths` arrays, singleton string values, and mixed-type arrays.
- [x] 2.2 Verify that Tinymist still activates and preserves variable substitution for valid arrays while ignoring malformed values without warning spam.
