...
(cd ../.. && docker build -t myriaddreamin/tinymist:0.13.18 .)
(cd samples && docker build -t myriaddreamin/tinymist-nvim:0.13.18 -f lazyvim-dev/Dockerfile .)
docker run --rm -it \
  ...
  -w /home/runner/dev myriaddreamin/tinymist-nvim:0.13.18 \
  $DOCKER_ARGS
