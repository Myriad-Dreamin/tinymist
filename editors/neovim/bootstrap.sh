...
(cd ../.. && docker build -t myriaddreamin/tinymist:0.13.19 .)
(cd samples && docker build -t myriaddreamin/tinymist-nvim:0.13.19 -f lazyvim-dev/Dockerfile .)
docker run --rm -it \
  ...
  -w /home/runner/dev myriaddreamin/tinymist-nvim:0.13.19 \
  $DOCKER_ARGS
