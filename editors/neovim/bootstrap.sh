...
(cd ../.. && docker build -t myriaddreamin/tinymist:0.13.43 .)
(cd samples && docker build -t myriaddreamin/tinymist-nvim:0.13.43 -f lazyvim-dev/Dockerfile .)
docker run --rm -it \
  ...
  -w /home/runner/dev myriaddreamin/tinymist-nvim:0.13.43 \
  $DOCKER_ARGS
