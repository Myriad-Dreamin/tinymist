...
(cd ../.. && docker build -t myriaddreamin/tinymist:0.14.19-rc2 .)
(cd samples && docker build -t myriaddreamin/tinymist-nvim:0.14.19-rc2 -f lazyvim-dev/Dockerfile .)
docker run --rm -it \
  ...
  -w /home/runner/dev myriaddreamin/tinymist-nvim:0.14.19-rc2 \
  $DOCKER_ARGS
