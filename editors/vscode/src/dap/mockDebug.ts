...
export class TypstDebugSession extends LoggingDebugSession {
  ...
  public constructor(fileAccessor: FileAccessor) {
    ...
    this._runtime.on("stopOnBreakpoint", () => {
      this.sendEvent(new StoppedEvent("breakpoint", TypstDebugSession.threadID));
    });
    this._runtime.on("stopOnDataBreakpoint", () => {
      this.sendEvent(new StoppedEvent("data breakpoint", TypstDebugSession.threadID));
    });
    this._runtime.on("stopOnInstructionBreakpoint", () => {
      this.sendEvent(new StoppedEvent("instruction breakpoint", TypstDebugSession.threadID));
    });
    this._runtime.on("stopOnException", (exception) => {
      if (exception) {
        this.sendEvent(new StoppedEvent(`exception(${exception})`, TypstDebugSession.threadID));
      } else {
        this.sendEvent(new StoppedEvent("exception", TypstDebugSession.threadID));
      }
    });
  ...
  protected async launchRequest(
    ...
    if (args.compileError) {
      ...
      this.sendErrorResponse(response, {
        ...
        showUser:
          args.compileError === "show" ? true : args.compileError === "hide" ? false : undefined,
      });
    } else {
      this.sendResponse(response);
    }
