...
class NightlyUtils {
  ...
  async updateTypstDependencies(typstVersion, typstAssetsRev) {
    ...
    try {
      updatedContent = edit(
        updatedContent,
        "workspace.dependencies.typst-assets.rev",
        typstAssetsRev,
      );
    } catch (e) {
  ...
  async updateSpecialFiles(newVersion) {
    ...
    try {
      ...
      nixContent = nixContent.replace(/version = "[^"]*";/g, `version = "${newVersion}";`);
...
`;
  ...
  calculateNewVersion(currentVersion, releaseType) {
    ...
    if (!validVersionRegex.test(currentVersion)) {
      throw new Error(
        `Invalid version format: ${currentVersion}. Version can only contain numbers, dots, hyphens, and 'rc'.`,
      );
    }
    ...
    if (releaseType === "canary") {
      ...
      if (currentPatch % 2 === 0) {
        throw new Error(
          `Current patch version ${currentPatch} is not odd. Nightly releases require odd patch versions.`,
        );
      }
    ...
    switch (command) {
      ...
      case "generate-changelog": {
        ...
        if (
          ...
          throw new Error(
            "Usage: generate-changelog <version> <tinymist-base-commit> <tinymist-base-message> <typst-rev> <typst-base-commit> <typst-base-message>",
          );
        }
        await utils.generateChangelog(
          newVersion,
          tinymistBaseCommit,
          tinymistBaseMessage,
          typstRev,
      ...
      default:
        ...
        console.error(
          "  generate-changelog <version> <tinymist-base-commit> <tinymist-base-message> <typst-rev> <typst-base-commit> <typst-base-message>",
        );
